...
class EducationClient(object):
    ...
    def _adddata_(self,usn,name,marks,add):
        self._address=_hash(FAMILY_NAME.encode("utf-8"))[0:6]+\
        _hash(usn.encode("utf-8"))[0:64]

        return self._wrap_and_send(name,marks,add)
        ...
        try:
            ...
            if not result.ok:
                raise Exception("Error {}: {}:{}".format(
                    result.status_code, result.reason,result.json()))
    ...
    def _wrap_and_send(self, name,marks,action, wait=15):
